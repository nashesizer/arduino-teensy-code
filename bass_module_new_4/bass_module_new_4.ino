// Working code for the base module of the Nashesizer - includes commented out workings - to tidy up
...
// This version integrates the joystick and rotary encoder modules and code from the 'MIDI_Serial_teensy_test1' sketch by Mike Cook to test 2-way comms
// Forwards joystick positions as emulated keypresses
// I2C two-way comms tested and working (might yet be refined) from the master Teensy 3.6 to/from the rotary encoder module's slave Arduino Nano (downstream)...
// and to/from the Processing Nashesizer-Hub OSC-to-Serial bridge to/from Live (upstream)
// Forwards encoder positions into Live and also receives and forwards on changes to LiveGrabber assigned GUI controls in Live to the rotary encoder module...
...
void handleHubMessage() {
  ...
  */
  ...
  // if the track # of the changed parameter in Live's interface is the same as the currently active track # - currently only works for track 2
...
void functionReadEncoders() {
  ...
  */
  ...
  if (encNoB == 0) { // Volume
    ...
    toHubMessageLast = toHubMessage;
  } else if (encNoB == 2) { // SendA
    // compile the message
    toHubMessage = "/T";
    toHubMessage += activeTrkNo;
    toHubMessage += "/SendA,f,";
    toHubMessage += encValueF;
    // print only if it's different to the last message
    if (toHubMessage != toHubMessageLast) {
      Serial.println(toHubMessage);
    ...
    toHubMessageLast = toHubMessage;
  } else if (encNoB == 3) { // SendB
    // compile the message
    toHubMessage = "/T";
    toHubMessage += activeTrkNo;
    toHubMessage += "/SendB,f,";
    toHubMessage += encValueF;
    // print only if it's different to the last message
    if (toHubMessage != toHubMessageLast) {
      Serial.println(toHubMessage);
